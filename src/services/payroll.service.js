...
export async function submitPayrollRunForReview(runId) {
  if (!runId) {
    throw new Error('submitPayrollRunForReview requires a payroll run id.')
  }

  const { data, error } = await supabase.rpc('submit_payroll_run_for_review', {
    p_run_id: runId,
  })
  if (error) throw error
...
export async function cancelPayrollRun(runId) {
  if (!runId) {
    throw new Error('cancelPayrollRun requires a payroll run id.')
  }

  const { data, error } = await supabase.rpc('cancel_payroll_run', {
    p_run_id: runId,
  })
  if (error) throw error

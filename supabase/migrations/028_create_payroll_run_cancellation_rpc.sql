-- 028_create_payroll_run_cancellation_rpc.sql
-- Cancel draft or reviewed payroll runs.
...
begin
  if not public.is_payroll() then
    raise exception 'Payroll run cancellation requires payroll role.'
      using errcode = '42501';
  end if;
  ...
  if current_status not in ('draft', 'review') then
    raise exception 'Only draft or reviewed payroll runs can be cancelled.'
      using errcode = '22023';
  end if;
...
revoke execute on function public.cancel_payroll_run(uuid) from public;
grant execute on function public.cancel_payroll_run(uuid) to authenticated;

comment on function public.cancel_payroll_run(uuid) is
  'Moves a draft or reviewed payroll run into cancelled status for callers with profiles.role = payroll and writes an audit log row.';
